...
mlo_width = [ 800, 800, 800]
mhi_width = [3500,3500, 3500]

### mlvj range min and max used when run with option --fitSignal
mlo_sig = [ 800, 800, 800]
mhi_sig = [ 3500,3500,3500]

mlo_sig_width = [800, 800, 800]
mhi_sig_width = [3500,3500,3500]
        ...
        for i in range(mLo,mHi):
            ...
            if options.limitMode==0:
              if options.isReReco == 0 and not options.noSys:
               runCmmd2 = "combine -M Asymptotic --minimizerAlgo Minuit2 --minosAlgo stepping -H ProfileLikelihood -m %03d -n _lim_%03d_bb_%s_ALLP -d whlvj_MWp_%03d_bb_%s_ALLP_unbin.txt -v 2"%(mass[i],mass[i],"mu",mass[i],"mu");
              elif options.isReReco == 0 and options.noSys:
               runCmmd2 = "combine -M Asymptotic --minimizerAlgo Minuit2 --minosAlgo stepping -H ProfileLikelihood -S 0 -m %03d -n _lim_%03d_bb_%s_ALLP -d whlvj_MWp_%03d_bb_%s_ALLP_unbin.txt -v 2"%(mass[i],mass[i],"mu",mass[i],"mu");
              elif options.isReReco == 1 and not options.noSys:
               runCmmd2 = "combine -M Asymptotic --minimizerAlgo Minuit2 --minosAlgo stepping -H ProfileLikelihood -m %03d -n _lim_%03d_bb_%s_ALLP -d whlvj_MWp_%03d_bb_%s_ALLP_unbin.txt -v 2"%(mass[i],mass[i],"mu",mass[i],"mu");
               print "LILILILILI"
#               runCmmd2 = "combine -M Asymptotic --minimizerAlgo Minuit2 --minosAlgo stepping -H ProfileLikelihood -m %03d -n _lim_%03d_bb_%s_ALLP -d whlvj_MWp_%03d_bb_%s_ALLP_counting.txt -v 2"%(mass[i],mass[i],"mu",mass[i],"mu");
              elif options.isReReco == 1 and options.noSys:
               runCmmd2 = "combine -M Asymptotic -S 0 --minimizerAlgo Minuit2 --minosAlgo stepping -H ProfileLikelihood -m %03d -n _lim_%03d_bb_%s_ALLP -d whlvj_MWp_%03d_bb_%s_ALLP_unbin.txt -v 2"%(mass[i],mass[i],"mu",mass[i],"mu");

              if options.batchMode:
               fn = "combineScript_Asymptotic_%s_%03d%s_%s"%("mu",mass[i],"","ALLP");
               submitBatchJobCombine(runCmmd2,fn,"mu",mass[i],"ALLP",options.isReReco);
              else:  
               os.system(runCmmd2);
        ...
        for i in range(mLo,mHi):            
            ### pvalue evaluation
              ...
              if options.isReReco == 0:   
               ...
               runCmmd += "combine -M ProfileLikelihood --significance --pvalue -m %03d -n _pval_exp_%03d_bb_%s_ALLP whlvj_MWp_%03d_bb_%s_ALLP_unbin.txt -t -1 --expectSignal=1 --toysFreq"%(mass[i],mass[i],"mu",mass[i],"mu");
              else: 
               runCmmd = "combine -M ProfileLikelihood --significance --pvalue -m %03d -n _pval_obs_%03d_bb_%s_ALLP whlvj_MWp_%03d_bb_%s_ALLP_unbin.txt\n"%(mass[i],mass[i],"mu",mass[i],"mu");
               runCmmd += "combine -M ProfileLikelihood --significance --pvalue -m %03d -n _pval_exp_%03d_bb_%s_ALLP whlvj_MWp_%03d_bb_%s_ALLP_unbin.txt --expectSignal=1 --toysFreq -t -1"%(mass[i],mass[i],"mu",mass[i],"mu");
              
              if options.batchMode:
               fn = "combineScript_HybridNew_%s_%03d%s_%s"%("mu",mass[i],"","ALLP");
               submitBatchJobCombine( runCmmd, fn, "mu",mass[i],"ALLP",options.isReReco);
              else:  
               os.system(runCmmd);
        ...
        for i in range(mLo,mHi):
            ...
            if options.limitMode ==2:
             ...
             if options.channel != "em":
              ...
              if options.batchMode:
               fn = "combineScript_HybridNew_%s_%03d%s_%s"%("mu",mass[i],"","ALLP");
               submitBatchJobCombine( runCmmd3,fn, "mu",mass[i],"ALLP" );
              else:  
               os.system(runCmmd3);
    ...
    if options.plotLimits:
        ...
        if options.channel != "em":
         ...
         for i in range(mLo,mHi):
             print "############################"+str(mass[i])+"#############################";            
             print "getting card: higgsCombine_pval_obs_%03d_bb_%s_ALLP.ProfileLikelihood.mH%03d.root"%(mass[i],"mu",mass[i]);

             orootname_mu_allp = "higgsCombine_pval_obs_%03d_bb_%s_ALLP.ProfileLikelihood.mH%03d.root"%(mass[i],"mu",mass[i]);

             #print "getting card: higgsCombine_pval_exp_%03d_bb_%s_ALLP.ProfileLikelihood.mH%03d.root"%(mass[i],"mu",mass[i]);

             #orootname_mu_allp_exp = "higgsCombine_pval_exp_%03d_bb_%s_ALLP.ProfileLikelihood.mH%03d.root"%(mass[i],"mu",mass[i]);
         ...
         if options.plotPvalue : 
          ...
          leg3.AddEntry( gr_mu_allp, "obs signif, #mu (ALLP)", "pl" );
	  ...
	  Extratext.Draw();
          ...
          ban3s.Draw();
          ban4s.Draw();
          can2.SaveAs("./LimitResult/Limit_ExpTail/pvals_mu_ALLP.pdf","pdf");
          can2.SaveAs("./LimitResult/Limit_ExpTail/pvals_mu_ALLP.png","png");
          can2.SaveAs("./LimitResult/Limit_ExpTail/pvals_mu_ALLP.root","root");
          can2.SaveAs("./LimitResult/Limit_ExpTail/pvals_mu_ALLP.C","C");
